...
//        ProcessBuilder p = new ProcessBuilder(command);
        ...
        String resp = "";
        ProcessBuilder builder = new ProcessBuilder("/home/uthpala/Uthpala/Uthpala/4th year/research/Shell cmd example/script.sh", id, id);
...
//                    body(stringStringMap);
    ...
    public ResponseEntity<Object> singleFileUpload(@RequestParam("file") MultipartFile file) {
         ...
         String command = "123";
        String result = "";
         String UPLOADED_FOLDER = "/home/uthpala/Uthpala/Uthpala/4th year/research/Shell cmd example/";
        if (file.isEmpty()) {
            Map<String, String> stringStringMap = new HashMap<>();
            stringStringMap.put("status", "failed");
        ...
        try {
           ...
           // ProcessBuilder builder = new ProcessBuilder("/home/uthpala/Uthpala/Uthpala/4th year/research/Shell cmd example/script.sh", command, command);
            ProcessBuilder builder = new ProcessBuilder("/home/uthpala/Uthpala/Uthpala/DockerVolume/BlockSci/docker/taint_run.sh");
            builder.redirectErrorStream(true);
            Process p = builder.start();
            BufferedReader r = new BufferedReader(new InputStreamReader(p.getInputStream()));
            ...
            Map<String, String> stringStringMap = new HashMap<>();
            stringStringMap.put("status", "success");
            stringStringMap.put("message", result);
            stringStringMap.put("code", "200");
            return ResponseEntity.status(HttpStatus.OK).
                    body(stringStringMap);
    }
